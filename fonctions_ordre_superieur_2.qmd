...
* Chaque **appel** de fonction (et non chaque définition) définit un environnement. Ici on crée donc deux environnements différents pour mystere.
...
Ce nom illustre un aspect de ces fonctions d'ordre supérieur, et une utilisation particulière. On a vu qu'il est possible d'écrire dans les environnements *parents* qui apparaissent comme isolés les uns des autres. Et bien utilisons cette propriété !
...
Seul `nouveau_compteur()` permet de créer un nouvel espace isolé. On appelle un tel espace **instance** et l'appel à `nouveau_compteur()` est appelée instanciation. **`compteur_1bis` réfère exactement au même compteur que 4**.
