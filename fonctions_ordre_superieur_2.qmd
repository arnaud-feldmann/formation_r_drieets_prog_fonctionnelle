...
Comment utiliser une closure pour faire une fonction `calcul_moins_dingue` qui renvoie les **mêmes valeurs** que `calcul_de_dingue` pour les **mêmes entrées** mais stocke les résultats dans une liste pour éviter de relancer les calculs plus d'une fois (on appelle cela un cache) ?
...
# Elle doit être appelée dans le code suivant
```
