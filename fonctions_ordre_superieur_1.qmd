...
Comme on le voit ci-dessous, lorsque l'on veut écrire vite dans la console, un raccourci pour `vapply` existe sans la `FUN.VALUE`. La fonction `sapply` devine automatiquement les dimensions et le type requis. Utiliser un `vapply` explicite est cependant *en général* une meilleure pratique à l'intérieur d'un programme.
