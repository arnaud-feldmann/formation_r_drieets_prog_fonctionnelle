...
Le code n'est interprêté qu'à temps d'exécution du code. Contrairement à des langages plus formels qui donnent une importance plus grande au temps de définition, notamment les langages compilés, R attend le dernier moment pour vérifier si quelque chose a un sens.
...
```{webr-r}
library(disaggR)
print(exists("disaggR"))
```

Lorsque l'on charge un package, par exemple [disaggR](https://github.com/InseeFr/disaggR), on utilise `library()` avec à l'intérieur un nom qui n'existe pourtant pas dans l'environnement global. Ici l'environnement global ne contient pas d'objet `disaggR`. C'est une forme d'évaluation non-standard puisque library ne réagit pas comme toutes les fonctions de R. Mais bon, ce n'est pas une évaluation non-standard des plus utiles en soi.
...
print(quosure_resultat(ma_quosure))
print(expression_resultat(mon_expression))
```
...
print(quosure_resultat(ma_quosure))
print(expression_resultat(mon_expression, environment()))
```
