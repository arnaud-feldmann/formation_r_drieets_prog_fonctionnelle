...
* On scentre sur **les fonctions plutôt que les classes d'objets**. Au lieu de s'attarder sur une classe puis de décrire les méthodes/fonctions que l'on peut appliquer dessus, dans S3 on fait *exactement l'inverse* ; on énonce un nom de fonction puis on décrit comment cette fonction s'applique sur différents objets. On a d'abord la fonction générique `faire_pleuvoir(objet)`. On l'implémente en `faire_pleuvoir.baguette_magique(objet)` qui est la méthode `faire_pleuvoir()` adaptée aux baguettes magiques. On peut ensuite implémenter également `faire_pleuvoir.robot_chanteur(objet)` si on veut prévoir une méthode `faire_pleuvoir()` adaptée aux robots chanteurs.
* Il n'y a **pas de notion d'instance d'objet**. La notion d'instance d'objet induit une notion d'unicité et d'état interne, comme on l'a vu au chapitre précédent. Si on copie `compteur_1` dans `compteur_1bis`, c'est le même compteur. Mais il n'y a pas de système d'instance natif à S3. Si on veut des *instances*, il faut les créer manu-militari de la même manière qu'on l'a déjà fait (c'est l'objet de l'exercice 2 de ce classeur).
