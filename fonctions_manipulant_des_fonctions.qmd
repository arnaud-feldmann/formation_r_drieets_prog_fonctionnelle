...
# Calcul 3 : Là encore, les valeurs de l'argument FUN.VALUE sont purement arbitraires.
...
La liste en entrée du `sapply` est une liste de vecteurs de tailles variables. On répète trois fois ses éléments. Les vecteurs en **sortie** seront donc de tailles variables également ; et c'est ce qui est rédhibitoire pour que le sapply puisse déterminer une dimension de matrice adéquate.

On ne peut pas résoudre le problème avec un `vapply`, mais le vapply a le mérite d'être plus strict : il renverra une erreur si les valeurs ne sont pas de la dimension explicitement demandée.
